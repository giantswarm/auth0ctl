package delete

import (
	"io"
	"os"

	"github.com/giantswarm/microerror"
	"github.com/giantswarm/micrologger"
	"github.com/spf13/cobra"

	a0client "github.com/giantswarm/auth0ctl/cmd/delete/client"
	a0resourceserver "github.com/giantswarm/auth0ctl/cmd/delete/resourceserver"
)

const (
	name        = "delete"
	description = "Commands to delete Auth0 resources."
)

type Config struct {
	Logger micrologger.Logger
	Stderr io.Writer
	Stdout io.Writer
}

func New(config Config) (*cobra.Command, error) {
	if config.Logger == nil {
		return nil, microerror.Maskf(invalidConfigError, "%T.Logger must not be empty", config)
	}
	if config.Stderr == nil {
		config.Stderr = os.Stderr
	}
	if config.Stdout == nil {
		config.Stdout = os.Stdout
	}

	var err error

	var clientCmd *cobra.Command
	{
		c := a0client.Config{
			Logger: config.Logger,
			Stderr: config.Stderr,
			Stdout: config.Stdout,
		}

		clientCmd, err = a0client.New(c)
		if err != nil {
			return nil, microerror.Mask(err)
		}
	}

	var resourceServerCmd *cobra.Command
	{
		c := a0resourceserver.Config{
			Logger: config.Logger,
			Stderr: config.Stderr,
			Stdout: config.Stdout,
		}

		resourceServerCmd, err = a0resourceserver.New(c)
		if err != nil {
			return nil, microerror.Mask(err)
		}
	}

	f := &flag{}

	r := &runner{
		flag:   f,
		logger: config.Logger,
		stderr: config.Stderr,
		stdout: config.Stdout,
	}

	c := &cobra.Command{
		Use:   name,
		Short: description,
		Long:  description,
		RunE:  r.Run,
	}

	f.Init(c)

	c.AddCommand(clientCmd)
	c.AddCommand(resourceServerCmd)

	return c, nil
}
